...
use polyref_checker_spi::cgroup::{IsolationBackend, PluginIsolationProfile, SeccompPolicy};
...
fn default_isolation_profile_matches_adr_009_security_defaults() {
    ...
    assert!(profile.seccomp.denied_syscalls.contains(&"ptrace".to_owned()));
    assert!(profile.seccomp.denied_syscalls.contains(&"mount".to_owned()));
    assert!(profile.seccomp.denied_syscalls.contains(&"chroot".to_owned()));
    assert!(profile.seccomp.denied_syscalls.contains(&"kexec_load".to_owned()));
}

#[test]
fn isolation_profile_rejects_network_enabled_v1() {
    let mut profile = PluginIsolationProfile::default();
...
#[test]
fn nsjail_args_include_no_network_and_resource_limits() {
    let profile = PluginIsolationProfile::default();
    let args = profile.backend_args(IsolationBackend::Nsjail).unwrap();

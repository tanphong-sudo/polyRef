//! JSON-RPC stdio adapter for the route checker plugin.

use polyref_checker_spi::checker::CheckRequest;
...
fn handle_request(request: JsonRpcRequest) -> JsonRpcResponse {
    ...
    match request.method.as_str() {
        "describe" => match serde_json::to_value(polyref_checker_route::describe_route_checker()) {
...
fn handle_check(request: JsonRpcRequest) -> JsonRpcResponse {
    let check_request = match serde_json::from_value::<CheckRequest>(request.params) {
        Ok(value) => value,
        Err(err) => {
            return error_response(
                request.id,
                -32602,
                "invalid check params",
                Some(serde_json::json!({ "detail": err.to_string() })),
            )
        }
    };
    match polyref_checker_route::check_route(&check_request) {
